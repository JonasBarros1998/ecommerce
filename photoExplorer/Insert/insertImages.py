...
class PhotoExplorer(IAuthenticationMongoDb, ErrorsMongoDb):
    ...
    def config(self):
        ...
        mongoClient = MongoClient(username=mongoUsername, password=mongoPassword)
        return mongoClient
    ...
    def _InsertManyImages(self, database, collection, datas):
        try:
            mongoClient = MongoClient(
                username="jonas", password="jonas-179406")
            database = mongoClient[database]
            collection = database[collection]
